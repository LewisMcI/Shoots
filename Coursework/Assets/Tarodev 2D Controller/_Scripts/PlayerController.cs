...
namespace TarodevController
{
    ...
    /*        if (IsServer && nextTime < Time.time)
            ...
            ApplyMovement();
            ApplyMovementServerRpc(_frameVelocity.x, _frameVelocity.y, this.NetworkObjectId);
        ...
        {
            // Simulate 
            //Debug.Log("the player " + theIDOftheCharacterThatMoves + " just moves from position");
            ApplyMovementClientRpc(_frameVelocityX, _frameVelocityY);
        }
        [ClientRpc]
        private void ApplyMovementClientRpc(float _frameVelocityX, float _frameVelocityY)
        {
            // Simulate for all players
            Vector2 frameVelocity = new Vector2(_frameVelocityX, _frameVelocityY);
